#!/usr/bin/env node
/**
 * 🖼️ Flamilha - Upload de Logo (CommonJS)
 * Execute: node upload-logo.cjs
 */
    ...
    req.on('end', () => {
      try {
        ...
        for (const part of parts) {
          if (part.includes('filename=')) {
            ...
            if (filenameMatch) {
              ...
              const contentStart = Buffer.from(part.substring(0, binaryStart)).length;
              fileBuffer = data.slice(contentStart, contentStart + (binaryEnd - binaryStart));
            }
          }
        }
        ...
        if (fileBuffer) {
          ...
          const savePath = path.join(UPLOAD_DIR, 'flamilha-logo' + ext);
